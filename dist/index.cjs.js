...
const refTable = tableJoin.tableJoin(countries, populations, [ID], tableJoin.LEFT);
const Filters = {
  get numeric() {
    return {
      population: numStrict.isNumeric,
      cases: numStrict.isNumeric
    };
  }
...
const Fields = {
  ...
  get incomeLevelSet() {
    return {
      side: INCOMELEVEL,
      sideFields: IncomeLevels
    };
  }

};
const regionsStat = async dataTable => {
  var _tableJoin2;

  // const dataTable = await Ncov.global({ top: 0 })
  const table$1 = (_tableJoin2 = tableJoin.tableJoin(dataTable, refTable, [ID], tableJoin.LEFT), table.Table.from(_tableJoin2));
  let t = chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.count));
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.cases)), [REGION], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.deaths)), [REGION], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.population)), [REGION], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.casesInMillion)), [REGION], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.deathsInMillion)), [REGION], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.regionSet, {}, Formulas.deathRate)), [REGION], tableJoin.LEFT);
  return table.Table.from(t).sort(CASES, comparer.NUM_DESC);
};
const incomeLevelsStat = async dataTable => {
  var _tableJoin3;

  // const dataTable = await Ncov.global({ top: 0 })
  const table$1 = (_tableJoin3 = tableJoin.tableJoin(dataTable, refTable, [ID], tableJoin.LEFT), table.Table.from(_tableJoin3));
  let t = chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.count));
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.cases)), [INCOMELEVEL], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.deaths)), [INCOMELEVEL], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.population)), [INCOMELEVEL], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.casesInMillion)), [INCOMELEVEL], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.deathsInMillion)), [INCOMELEVEL], tableJoin.LEFT);
  t = tableJoin.tableJoin(t, chipStat(table$1, _objectSpread2({}, Fields.incomeLevelSet, {}, Formulas.deathRate)), [INCOMELEVEL], tableJoin.LEFT);
  return table.Table.from(t).sort(CASES, comparer.NUM_DESC);
};

const chipStat = (table$1, {
  side,
  field,
  filter,
  formula,
  ...
  if (table$1.coin('_') < 0) table$1.unshiftColumn('_', vectorInit.iso(table$1.ht, ''));
  ...
  if (sideFields) vectorMapper.mutate(crostab.side, x => {
    var _sideFields$x;

    return (_sideFields$x = sideFields[x]) !== null && _sideFields$x !== void 0 ? _sideFields$x : x;
  });
  const title = phrasing.adjoin(indicator, 'by', side);
  const head = [side, indicator];
  const rows = entriesInit.wind(crostab.side, crostab.rows.map(([v]) => v));
  return table.Table.from({
    head,
    rows,
    title
  }).sort(indicator, comparer.NUM_DESC);
};
...
class NcovCli {
  static async start() {
    ...
    if (scope === STAT) {
      var _ref2, _ref3, _await$regionsStat, _ref4, _ref5, _await$incomeLevelsSt, _ref6;
      ...
      _ref3 = (_await$regionsStat = await regionsStat(table), logger.decoTable(_await$regionsStat)), logger.says[REGION](_ref3);
      _ref4 = '', logger.logger(_ref4);
      _ref5 = (_await$incomeLevelsSt = await incomeLevelsStat(table), logger.decoTable(_await$incomeLevelsSt)), logger.says[INCOMELEVEL](_ref5);
    ...
    await Ncov[scope]({
      ...
      var _ref7, _result, _ref8, _result2;

      spn.succeed(xr.Xr('updated')['scope'](scope)['timestamp'](timestamp.now()).toString());
      if (format === enumTabularTypes.TABLE) _ref7 = (_result = result, logger.DecoTable({
        read: x => typeof x === enumDataTypes.NUM ? mag.format(x) : decoFlat.decoFlat(x)
      })(_result)), logger.says['corona latest report'].br(scope)(_ref7);
      if (format === enumTabularTypes.SAMPLES) _ref8 = (_result2 = result, logger.decoSamples(_result2)), logger.says['corona latest report'](_ref8);
    });
    _ref9 = '', logger.logger(_ref9);
  }
