...
const c12ns = tableJoin.tableJoin(countries, populations, [ID], enumJoinModes.LEFT); // c12ns |> decoTable |> logger
...
const groupedStat = async (table$1, {
  ...
  table$1 = table.Table.from(tableJoin.tableJoin(table$1, c12ns, [ID], enumJoinModes.LEFT)).group({
    key: groupBy,
...
class NcovCli {
  static async start() {
    ...
    if (scope === STAT) {
      ...
      const spn = ora(xr.Xr('updating').timestamp(timestampPretty.time()).toString()).start();
    ...
    const spn = ora(xr.Xr('updating').sortBy(sortBy).top(top).timestamp(timestampPretty.time()).toString()).start();
    await Ncov[scope]({
      format,
      sortBy,
      top,
      ...
      if (format === enumTabularTypes.SAMPLES) _ref10 = (_result2 = result, logger.decoSamples(_result2)), logger.says['corona latest report'].br(scope)(_ref10);
    });
